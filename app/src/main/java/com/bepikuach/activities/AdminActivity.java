...
import com.bepikuach.services.WatermarkService;
...
public class AdminActivity extends AppCompatActivity {
    ...
    private void restartWatermark() {
        Intent i = new Intent(this, WatermarkService.class);
        stopService(i);
        if (prefs.isWatermarkEnabled()) startService(i);
    }

    private void setupWatermark() {
        android.widget.Switch sw     = findViewById(R.id.switchWatermark);
        android.widget.SeekBar seekA = findViewById(R.id.seekWmAlpha);
        android.widget.SeekBar seekS = findViewById(R.id.seekWmSize);
        android.widget.TextView tvA  = findViewById(R.id.tvWmAlpha);
        android.widget.TextView tvS  = findViewById(R.id.tvWmSize);
        android.widget.Button btnTop    = findViewById(R.id.btnWmTop);
        android.widget.Button btnCenter = findViewById(R.id.btnWmCenter);
        android.widget.Button btnBottom = findViewById(R.id.btnWmBottom);
        android.widget.Button btnLeft   = findViewById(R.id.btnWmLeft);
        android.widget.Button btnRight  = findViewById(R.id.btnWmRight);

        sw.setChecked(prefs.isWatermarkEnabled());
        seekA.setProgress(prefs.getWatermarkAlpha());
        seekS.setProgress(prefs.getWatermarkSizeDp());
        tvA.setText(prefs.getWatermarkAlpha() + "%");
        tvS.setText(prefs.getWatermarkSizeDp() + "dp");
        highlightWmGravity(prefs.getWatermarkGravity(), btnTop, btnCenter, btnBottom, btnLeft, btnRight);

        sw.setOnCheckedChangeListener((b, checked) -> {
            prefs.setWatermarkEnabled(checked);
            restartWatermark();
        });

        seekA.setOnSeekBarChangeListener(new android.widget.SeekBar.OnSeekBarChangeListener() {
            public void onProgressChanged(android.widget.SeekBar s, int p, boolean u) {
                prefs.setWatermarkAlpha(p);
                tvA.setText(p + "%");
                restartWatermark();
            }
        ...
        seekS.setOnSeekBarChangeListener(new android.widget.SeekBar.OnSeekBarChangeListener() {
            public void onProgressChanged(android.widget.SeekBar s, int p, boolean u) {
                prefs.setWatermarkSizeDp(p);
                tvS.setText(p + "dp");
                restartWatermark();
            }
            public void onStartTrackingTouch(android.widget.SeekBar s) {}
        ...
        android.view.View.OnClickListener gravityClick = v -> {
            int g = 0;
            if      (v.getId() == R.id.btnWmTop)    g = 1;
            else if (v.getId() == R.id.btnWmBottom)  g = 2;
            else if (v.getId() == R.id.btnWmLeft)    g = 3;
            else if (v.getId() == R.id.btnWmRight)   g = 4;
            prefs.setWatermarkGravity(g);
            highlightWmGravity(g, btnTop, btnCenter, btnBottom, btnLeft, btnRight);
            restartWatermark();
        };
        btnTop.setOnClickListener(gravityClick);
        btnCenter.setOnClickListener(gravityClick);
        btnBottom.setOnClickListener(gravityClick);
        btnLeft.setOnClickListener(gravityClick);
        btnRight.setOnClickListener(gravityClick);
    }

    private void highlightWmGravity(int g, android.widget.Button top,
            android.widget.Button center, android.widget.Button bottom,
            android.widget.Button left, android.widget.Button right) {
        int active = 0xFF1A237E, inactive = 0xFF9E9E9E;
        top.setBackgroundColor(g == 1 ? active : inactive);
        center.setBackgroundColor(g == 0 ? active : inactive);
        bottom.setBackgroundColor(g == 2 ? active : inactive);
        left.setBackgroundColor(g == 3 ? active : inactive);
        right.setBackgroundColor(g == 4 ? active : inactive);
    }
